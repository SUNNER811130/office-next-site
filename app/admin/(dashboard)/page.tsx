...
const sections = [
  ...
  { href: "/admin/pages/contact", label: "聯絡頁區塊", description: "聯絡頁區塊顯示、順序、背景、版型與動畫" },
  ...
  return (
    <div className="grid gap-5">
      <section className="grid gap-5 lg:grid-cols-3">
        ...
        <article className="rounded-[1.8rem] border border-ink/8 bg-white/86 p-5"><p className="text-[11px] uppercase tracking-[0.28em] text-bronze">Page Blocks</p><p className="mt-3 text-xl font-medium text-ink">聯絡頁區塊管理</p><p className="mt-2 text-sm text-slate">目前啟用 {content.pageBlocks.contact.filter((block) => block.enabled).length} 個區塊</p><Link href="/admin/pages/contact" className="mt-4 inline-flex rounded-full border border-ink/10 px-4 py-2 text-sm text-ink">前往聯絡頁區塊</Link></article>
        <article className="rounded-[1.8rem] border border-ink/8 bg-white/86 p-5">
