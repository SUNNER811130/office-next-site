...
右側預覽可切換 390px 手機、768px 平板與 1280px 桌機，也可切換「已發布版本」與「草稿預覽」、手動刷新或在新分頁開啟目前模式。本地修改不會進入 iframe；成功 Save Draft／Reset Draft 後才可切到草稿，Discard 或 Publish 後回到 Published。Home Draft Preview 只讀 `pageBlocks.home`，不會混入 Services／About／Contact Page Block Draft。
...
右側 iframe 可切換 390px、768px、1280px、Published／Draft，並可手動刷新或在新分頁開啟。Save Draft 與 Reset Draft 成功後會更新 Draft Preview；Publish 後回到已發布版本。
...
右側 iframe 可切換 390px、768px、1280px 與 Published／Draft。Save Draft 與 About Reset Draft 成功後可預覽未發布設定；Discard／Publish 後回到 Published。
...
右側 iframe 可切換 390px、768px、1280px 與 Published／Draft，亦可手動刷新或在新分頁開啟。Save Draft／Reset Draft 成功後更新 Draft Preview；Discard／Publish 後回到 Published。隱藏區塊只改變呈現，不會刪除 Email、Contact Intro、洽詢選項、品牌內容或 Social Links。
...
四頁後台共用 `components/admin/page-block-editor/` 的單一架構。`PageBlockEditor` 管理 blocks、排序、顯示切換、Reset Draft 與 preview snapshot；`usePageBlockWorkflow` 管理 snapshot、dirty、operation、conflict、duplicate guard、AbortController 與 mounted guard；`page-block-workflow-helpers` 提供 typed nested API adapter。Status、actions、conflict 與 confirmation dialog 重用 L4 元件；`PageBlockPreview` 再重用 Admin 共用的 Published／Draft iframe，保留既有 accessibility、allowlists 與 390／768／1280 行為。
