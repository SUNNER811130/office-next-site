  ...
  return <div className="grid min-w-0 gap-6 xl:grid-cols-[minmax(0,1fr)_minmax(360px,0.82fr)]">
    <div className="grid min-w-0 gap-4">
      ...
      <div className="rounded-[2rem] border border-red-900/15 bg-white/75 p-5">
        ...
        {resetOpen ? <ContentWorkflowConfirmDialog id={`reset-${config.page}-blocks`} title={`將${config.pageLabel}預設 Page Blocks 建立為草稿？`} confirmLabel="確認建立 Reset Draft" tone="danger" onCancel={() => setResetOpen(false)} onConfirm={() => { setResetOpen(false); void workflow.reset(); }}><p>{config.resetConfirmation}</p><p>恢復預設會先建立草稿，不會立即更新公開網站。</p><p>現有未發布草稿與本地修改會由目前頁的預設值取代。</p></ContentWorkflowConfirmDialog> : null}
      </div>
